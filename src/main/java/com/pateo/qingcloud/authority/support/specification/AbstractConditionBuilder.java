...
public abstract class AbstractConditionBuilder<T> {
    ...
    private Comparable processMaxValueOnDate(Comparable maxValue) {
        if(maxValue instanceof Date) {
			LocalDate localDate=TimeUtils.UDateToLocalDate((Date) maxValue);
			LocalDateTime localDateTime =LocalDateTime.of(localDate, LocalTime.of(11, 59, 59));
			maxValue=TimeUtils.LocalDateTimeToUdate(localDateTime);
           //  new DateTime(maxValue).withTimeAtStartOfDay().plusDays(1).plusSeconds(-1).toDate();
        }
        return maxValue;
    }
